    ...
    public class LoginInteractorTest {
        ...
        public void testLoginFailureWithIncorrectPassword() {
            // Assuming 'employee' is an existing user with a different password
            LoginInputData inputData = new LoginInputData(employee.getUID(), "wrongPassword");
            LoginOutputSpy mockPresenter = new LoginOutputSpy();
            LoginInteractor loginInteractor = new LoginInteractor(mockPresenter, employeeDAO);
            loginInteractor.execute(inputData);

            assertFalse(mockPresenter.isSuccessCalled());
            assertTrue(mockPresenter.isFailCalled());
        }

        @Test
        public void testLoginFailureWithNonexistentUser() {
            LoginInputData inputData = new LoginInputData("nonexistentUser", "somePassword");
            LoginOutputSpy mockPresenter = new LoginOutputSpy();
            LoginInteractor loginInteractor = new LoginInteractor(mockPresenter, employeeDAO);
            loginInteractor.execute(inputData);
